...
Retrieval-augmented Generation (RAG) has become the go-to tool for any professional that wants to combine the power Large Language Models (LLMs) and proprietary data. RAG makes external data available to LLMs in a similar way that a prompter is helping actors on stage to remember their scripts. Even if a model has never seen your company’s data during training, RAG can address this knowledge gap by fetching relevant information from your database. Since the application of RAG in industry settings is a rather new trend that has only started to emerge in 2023, it’s hard to get robust data on its effects outside of academia¹. But following blog posts and expert opinions on LinkedIn an Twitter, RAG has without a doubt become one of the most discussed topics within the AI community. And the main driver for this rise in popularity is the boogeyman that haunts each and every LLM to this day: hallucinations.

### What hallucinations are and why they are dangerous

Hallucination - or more precisely, machine hallucination - is a term for a a piece of information that is incorrectly made up by a generative model. It’s an anthropomorphizing term that some argue to be more harm- than helpful². But if we think about hallucinations in laypeople terms, “seeing something that isn’t there” works as an analogy. And following this mental image, it should be immediately clear why hallucinations are bad for business. They are often named as one of the biggest blockers and concerns for industry adoption of Generative AI and LLMs in particular³. Google including made up information in their slides when presenting their newest Chatbot advancements caused Alphabet’s stock to fall for 7% in February⁴. While the stock has recovered since and is doing better than ever, this has shown how sensitive the public can be to incorrect information generated by AI. This problem becomes even more severe when LLMs are put into production for uses cases that aren’t as harmless as chit-chatting with a user or looking up restaurants on the internet.

### Retrieval and augmentation: a love story
...
Great. Next we will transform the PDF into texts and store them as vector embeddings in a basic vector store. We then perform a simple similarity search on our documents (aka retrieval).
...
I will only show the relevant part of the documents here for aesthetic reasons, so keep in mind that the following is *not* the full output of docs:
...
Finally, we will use the same tokenization and generation pipeline again as we did earlier, just this time we add the context we retrieved in front of our question.
...
Et voilà! If you go back to the passage from the original document, this is the correct answer. More than 10x higher than what our model hallucinated earlier - RAG saved us from what could've lead to a terrible decision. 

### Conclusion

I hope that I could make clear how retrieval can add value to generation. It's not a silver bullet, but it has a lot of advantages and few disadvantages. Only in rare cases does retrieving relevant context degrade the quality of a model's response. Put simply: you have to make sure that the documents you're using do *actually* include the answers you're looking for. Irrelevant context can hurt the generative process. As with all things Data Science: garbage in, garbage out.

### Sources:

¹https://github.com/myscale/retrieval-qa-benchmark

²https://betterprogramming.pub/large-language-models-dont-hallucinate-b9bdfa202edf

³https://fortune.com/2023/04/17/google-ceo-sundar-pichai-artificial-intelligence-bard-hallucinations-unsolved/

⁴https://www.cnbc.com/2023/02/08/alphabet-shares-slip-following-googles-ai-event-.html
