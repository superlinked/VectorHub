...
Retrieval-augmented Generation (RAG) balances information retrieval, which finds existing information, with generation, which creates new content. RAG achieves this by combining Large Language Models (LLMs) with external data sources and information retrieval algorithms. Relevant context is retrieved from external data in order to prevent incorrect generations. This makes RAG a valuable tool in applied settings across various industries, including legal, education, and finance.

## RAG's rapid rise in popularity

RAG has become the go-to tool for any professional that wants to combine the power LLMs with their proprietary data. RAG makes external data available to LLMs the way a prompter helps actors on stage remember their lines. Even if a model is unable to produce a correct answer to a question by itself, RAG can address this by fetching relevant information from a database. Because applying RAG in industry settings is relatively new - really beginning just this year (2023), there's not much robust data on its performance on real-world problems [outside of academia](https://github.com/myscale/retrieval-qa-benchmark). But judging by LinkedIn and Twitter posts by experts, RAG has become one of the most discussed topics within the AI community. And the main driver for these discussions is the bogeyman that continues to haunt all generative LLMs: hallucinations.
