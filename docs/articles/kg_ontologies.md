...
One of the keys to a knowledge graph’s (KG) power is its ontology. **The ontology makes the KG intelligible**. It acts as a schema layer elucidating the basic structure of the KG by defining the types of entities and relationships that can exist in the graph. We are familiar with the concept of schemas from relational databases. In a database, we have the data in the table and the metadata or schema that describes the columns of that table. In knowledge graphs, everything is considered data, allowing seamless movement between metadata and data. Instead of a schema of tables and columns, knowledge graphs use a more powerful concept called an ontology.
